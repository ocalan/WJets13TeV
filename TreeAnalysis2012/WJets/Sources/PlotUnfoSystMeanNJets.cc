...
//--- Declaring functions -------------
void PlotUnfoSystMeanNJets(string var = "MeanNJetsHT_Zinc1jet");
void FuncPlot(string variable = "MeanNJetsHT_Zinc1jet", bool log = 1, bool decrease = 1);
//--------------------------------
...
void PlotUnfoSystMeanNJets(string var)
{
    TH1::SetDefaultSumw2();
    TH2::SetDefaultSumw2();
    ...
    cout << " Processing variable: " << varStruct[j].name << endl;
    cout << varStruct[j].name << " " <<  varStruct[j].log << " " << varStruct[j].decrease << endl;
    
    FuncPlot(varStruct[j].name, varStruct[j].log, varStruct[j].decrease);
...
void FuncPlot(string variable, bool logZ, bool decrease)
{
    ...
    for (int i = 0 ; i < 7 ; i ++){
        ...
        TLegend *leg = new TLegend(xLowLeg, 0.7, xHighLeg, 0.89);
        leg->SetFillStyle(0);
        leg->SetBorderSize(0);
        ...
        hSyst[2*i]->GetXaxis()->SetTitle(tempXTitle.c_str());
        
        if (i != 0){
            //hSyst[2*i]->GetYaxis()->SetRangeUser(0.81, 1.19);
            hSyst[2*i]->GetYaxis()->SetRangeUser(0.91, 1.09);
    ...
    for (int k = 14; k < nSyst; k++){
        ...
        hSyst[k]->GetXaxis()->SetTitle(tempXTitle.c_str());
        
        //hSyst[k]->GetYaxis()->SetRangeUser(0.81, 1.19);
        hSyst[k]->GetYaxis()->SetRangeUser(0.91, 1.09);
